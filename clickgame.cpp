...
clickgame::clickgame() {
    ...
    // 根據餘數初始化怪物等級與屬性
    string mname = "怪物"; // 這裡可以使用更具體的名字
    Monster monster(mname, remainder + 1);
...
void clickgame::playGame(Player &name) {
    ...
    currentVisibleLines = monster.size();

    cout << "===== 鍵盤連點遊戲 =====" << endl;
    cout << "怪物等級：" << monsterlv << endl;
    cout << "總血量：" << monsterHP << endl;
    cout << "你的攻擊力：" << name.getatk() << endl;
    cout << "你有 " << durationInSeconds << " 秒的時間擊敗怪物！" << endl;
    ...
    if (monsterHP > 0) {
        cout << "挑戰失敗！怪物還有 " << monsterHP << " HP。" << endl;
    }
}
